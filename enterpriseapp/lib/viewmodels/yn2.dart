...
class YN2VM extends ChangeNotifier {
  ...
  YN2VM(this.sessionProvider, this.configProvider) {
    _setTitle();
  ...
  void _setTitle() {
    ...
    switch (sessionProvider.session.lang) {
      case 'en':
        title = configProvider.config!.qyn2;
        break;
      case 'es':
        title = configProvider.config!.altqyn2;
        break;
      default:
        title = configProvider.config!.qyn2;
    }
    _title = title ?? 'Yes/No Question 2';
  }

  // Validate the name and navigate to the next screen
  void submitScreen(String answer) {
    // Update session with the name
    sessionProvider.session.ayn2 = answer;
